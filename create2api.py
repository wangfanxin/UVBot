...
class Create2(object):
    ...
    #------------------------ Self Define ----------------------------
    def get_sensors_uvbot(self):
        sensor = self.get_sensors()
        light = {
            'L': sensor.light_bumper_left,
            'LF':sensor.light_bumper_front_left,
            'LC':sensor.light_bumper_center_left,
            'CR':sensor.light_bumper_center_right,
            'RF':sensor.light_bumper_front_right,
            'R' :sensor.light_bumper_right
        }
        bump = {
            'L' :sensor.bumps_wheeldrops.bump_left,
            'R' :sensor.bumps_wheeldrops.bump_right,
            'any' :sensor.bumps_wheeldrops.bump_right or sensor.bumps_wheeldrops.bump_left
        }
        encoder = {
            'L' :sensor.encoder_counts_left,
            'R' :sensor.encoder_counts_right
        }
        return light, bump, encoder